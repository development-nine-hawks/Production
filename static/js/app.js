...
async function pgGen(el) {
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white rounded-xl shadow-sm border p-6"><h2 class="text-lg font-semibold mb-4">Pattern Settings</h2>
            <form id="gf" class="space-y-4">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Block Size <span class="text-gray-400 font-normal">(PRNG macro-block px)</span></label><select id="gbs" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none"><option value="4">4 px (fine detail)</option><option value="8" selected>8 px (default)</option><option value="16">16 px (coarse)</option><option value="32">32 px (very coarse)</option></select></div>
    ...
    document.getElementById('gf').addEventListener('submit', async e => {
        ...
        try {
            ...
            const r = await API.post('/api/patterns/generate', { serial_number: document.getElementById('gs').value, label: document.getElementById('gl').value, seed: sv ? parseInt(sv) : null, block_size: parseInt(document.getElementById('gbs').value), notes: document.getElementById('gn').value });
            document.getElementById('gp').innerHTML = `<div class="flex justify-between items-center mb-4"><h2 class="text-lg font-semibold">Generated Pattern</h2><div class="flex gap-2"><div class="relative" id="dl-dd"><button onclick="document.getElementById('dl-menu').classList.toggle('hidden')" class="inline-flex items-center gap-1.5 bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Download<svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg></button><div id="dl-menu" class="hidden absolute right-0 mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-10 py-1"><a href="/api/patterns/${r.id}/download" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">PNG Image</div><div class="text-xs text-gray-400">Raw ${r.pattern_size}x${r.pattern_size}px pattern</div></a><div class="border-t border-gray-100 my-1"></div><a href="/api/patterns/${r.id}/pdf?size_mm=15" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">PDF - 15mm x 15mm</div><div class="text-xs text-gray-400">A4 page, pattern centered</div></a><a href="/api/patterns/${r.id}/pdf?size_mm=7.5" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">PDF - 7.5mm x 7.5mm</div><div class="text-xs text-gray-400">A4 page, pattern centered</div></a><div class="border-t border-gray-100 my-1"></div><a href="/api/patterns/${r.id}/label-png" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">Label PNG</div><div class="text-xs text-gray-400">NINEHAWK branded label image</div></a><a href="/api/patterns/${r.id}/label-pdf?size_mm=15" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">Label PDF - 15mm</div><div class="text-xs text-gray-400">NINEHAWK branded label</div></a><a href="/api/patterns/${r.id}/label-pdf?size_mm=7.5" download class="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><div class="font-medium">Label PDF - 7.5mm</div><div class="text-xs text-gray-400">NINEHAWK branded label</div></a></div></div><a href="#/verify" class="inline-flex items-center gap-1.5 bg-white text-brand-700 px-4 py-2 rounded-lg text-sm font-medium border border-brand-200 hover:bg-brand-50 transition">Verify This</a></div></div><div class="flex justify-center"><img src="/api/patterns/${r.id}/preview" class="max-w-md w-full rounded-lg shadow-md cursor-pointer border" onclick="openImageModal(this.src)"/></div><div class="mt-4 grid grid-cols-2 gap-4 text-sm"><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Serial:</span> <span class="font-medium">${r.serial_number}</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Seed:</span> <span class="font-mono font-medium">${r.seed}</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Size:</span> <span class="font-medium">${r.pattern_size}x${r.pattern_size}px</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Block:</span> <span class="font-medium">${r.block_size}px</span></div><div class="bg-gray-50 rounded-lg p-3 col-span-2"><span class="text-gray-500">Created:</span> <span class="font-medium">${fmtDate(r.created_at)}</span></div></div>`;
...
async function pgVerify(el) {
    el.innerHTML = `
    <div class="mb-8">
        <h1 class="text-2xl font-bold">Verify Pattern</h1>
        <p class="text-gray-500 mt-1">Photograph the label — the seed is decoded automatically from the Data Matrix codes</p>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 flex flex-col gap-5">
            ...
            <div class="bg-white rounded-2xl shadow-sm border p-6">
                <div id="dz" class="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center
                     hover:border-brand-400 hover:bg-brand-50/20 transition cursor-pointer
                    ...
                    <div>
                        <p class="font-semibold text-gray-700">Drop photo here</p>
                        <p class="text-sm text-gray-400 mt-0.5">or click to browse &nbsp;·&nbsp; JPG / PNG</p>
                    </div>
                    <input type="file" id="vf" accept="image/*" multiple class="hidden"/>
                </div>

                <!-- Preview thumbnails -->
                <div id="fl" class="mt-4 hidden space-y-2"></div>

                <!-- Optional fields -->
                <div id="opt-fields" class="mt-4 hidden space-y-3">
                    <div class="flex gap-3">
                        <div class="flex-1">
                            <label class="block text-xs text-gray-500 mb-1">Print size (mm)</label>
                            <input type="number" id="vs" placeholder="e.g. 65"
                        ...
                        <div class="flex-1">
                            ...
                            <input type="text" id="vn" placeholder="Optional"
                                class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-brand-500 outline-none"/>
                        </div>
                    </div>
                </div>
            ...
            <!-- Verify button -->
            <button id="vb" disabled
                class="w-full bg-brand-600 text-white py-3.5 rounded-xl font-semibold
                       hover:bg-brand-700 transition disabled:bg-gray-200 disabled:text-gray-400
            ...
            <!-- How it works hint -->
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 text-xs text-indigo-700 space-y-1.5">
                <p class="font-semibold text-indigo-800">How it works</p>
                <p>① DMs decoded → seed extracted</p>
                <p>② Pattern region cropped using DM geometry</p>
                <p>③ Reference regenerated from seed</p>
                <p>④ Moire / correlation / color / gradient scored</p>
            </div>
        </div>

        <!-- Right: results -->
        <div class="lg:col-span-3">
            <div id="vr" class="bg-white rounded-2xl shadow-sm border p-6">
    ...
    const dz = document.getElementById('dz'), fi = document.getElementById('vf'), vb = document.getElementById('vb');
    let files = [];
        ...
        if (files.length) {
            ...
            fl.innerHTML = files.map(file => {
                ...
                return `<div class="flex items-center gap-3 bg-gray-50 rounded-xl p-2">
                    <img src="${url}" class="w-14 h-14 rounded-lg object-cover border"/>
    ...
    vb.addEventListener('click', async () => {
        ...
        vr.innerHTML = spin('Decoding Data Matrix codes…');

        try {
            const sz = document.getElementById('vs').value, nt = document.getElementById('vn').value;

            if (files.length === 1) {
                // ── Single file ────────────────────────────────────────────
                const fd = new FormData();
                fd.append('captured', files[0]);
                if (sz) fd.append('print_size_mm', sz);
                ...
                // ── Batch ─────────────────────────────────────────────────
                const fd = new FormData();
                files.forEach(f => fd.append('captured_files', f));
                if (sz) fd.append('print_size_mm', sz);
                if (nt) fd.append('notes', nt);
                ...
                <div class="flex justify-between items-start mb-5">
                  <div>
                    <h2 class="text-lg font-bold">Batch Results</h2>
                    <p class="text-xs text-gray-400">${d.results.length} photos submitted</p>
                  </div>
                  <div class="text-right">
                    <div class="text-2xl font-bold ${ok===d.results.length?'text-green-600':'text-yellow-600'}">${ok}/${d.results.length}</div>
                    <div class="text-xs text-gray-400">passed</div>
                  </div>
                </div>
                <div class="space-y-2">
                ${d.results.map(r=>`
                  <div class="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 ${r.id?'cursor-pointer hover:bg-gray-100':''}"
                       ${r.id?`onclick="location.hash='#/results/${r.id}'"`:''}>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-medium truncate">${r.filename}</p>
                      <p class="text-xs text-gray-400">${r.verdict!=='ERROR'?`Confidence: ${(r.confidence*100).toFixed(1)}%`:r.error||'Error'}</p>
                    </div>
                    ${vBadge(r.verdict)}
                  </div>`).join('')}
                </div>`;
            ...
            vr.innerHTML = `<h2 class="text-lg font-semibold mb-4">Results</h2>
            <div class="bg-red-50 border border-red-200 rounded-xl p-4 text-red-800 text-sm">${err.message}</div>`;
            showToast(err.message, 'error');
        } finally {
            vb.disabled = false;
            vb.innerHTML = `<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
async function pgDetail(el, p) {
    ...
    try {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2"><div class="bg-white rounded-xl shadow-sm border p-5"><h3 class="font-semibold mb-4">Image Comparison</h3><div class="grid grid-cols-4 gap-4"><div><p class="text-sm text-gray-500 mb-2 text-center font-medium">Original</p><img src="/api/verify/${r.id}/images/original" class="w-full rounded-lg border cursor-pointer shadow-sm hover:shadow-md transition" onclick="openImageModal(this.src)"/></div><div><p class="text-sm text-gray-500 mb-2 text-center font-medium">Captured</p><img src="/api/verify/${r.id}/images/captured" class="w-full rounded-lg border cursor-pointer shadow-sm hover:shadow-md transition" onclick="openImageModal(this.src)"/></div><div><p class="text-sm text-gray-500 mb-2 text-center font-medium">Cropped</p><img src="/api/verify/${r.id}/images/roi" class="w-full rounded-lg border cursor-pointer shadow-sm hover:shadow-md transition" onclick="openImageModal(this.src)" onerror="this.style.display='none';this.parentElement.insertAdjacentHTML('beforeend','<div class=\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-12 text-sm text-gray-400\'>Expired</div>')"/></div><div><p class="text-sm text-gray-500 mb-2 text-center font-medium">Aligned</p><img src="/api/verify/${r.id}/images/aligned" class="w-full rounded-lg border cursor-pointer shadow-sm hover:shadow-md transition" onclick="openImageModal(this.src)" onerror="this.style.display='none';this.parentElement.insertAdjacentHTML('beforeend','<div class=\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-12 text-sm text-gray-400\'>Expired</div>')"/></div></div></div><div class="bg-white rounded-xl shadow-sm border p-5 mt-5"><div class="flex justify-between items-center"><div><h3 class="font-semibold">Quick Re-test</h3><p class="text-sm text-gray-500 mt-0.5">Verify another photo against the same pattern</p></div><a href="#/verify" class="bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700">New Verification</a></div></div></div>
        <div class="bg-white rounded-xl shadow-sm border p-5 mt-5">
            <div class="flex justify-between items-center mb-3">
                <div><h3 class="font-semibold">Detection &amp; Alignment</h3><p class="text-sm text-gray-500 mt-0.5">DataMatrix corners (green = Top, blue = Right) → pattern region (red), warped by an 8-point homography</p></div>
                <span class="text-xs font-medium px-2.5 py-1 rounded-full ${r.align_method==='dm_corners'?'bg-green-100 text-green-700':(r.align_method?'bg-amber-100 text-amber-700':'bg-gray-100 text-gray-500')}">${r.align_method==='dm_corners'?'DM-corner align':(r.align_method?'rough-crop fallback':'—')}</span>
            </div>
            <img src="/api/verify/${r.id}/images/detection" class="w-full rounded-lg border cursor-pointer hover:shadow-md transition" onclick="openImageModal(this.src)" onerror="this.style.display='none';this.parentElement.insertAdjacentHTML('beforeend','<div class=\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-12 text-sm text-gray-400\'>Detection image unavailable</div>')"/>
        </div>

        <div class="bg-white rounded-xl shadow-sm border p-5 mt-5">
            <div class="flex justify-between items-center">
                <div><h3 class="font-semibold">Test Case Result</h3><p class="text-sm text-gray-500 mt-0.5">Copy result to paste into TEST_CASES.md</p></div>
                <button onclick="copyTestResult()" class="inline-flex items-center gap-2 bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-900 transition">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-5 mt-5">
            <div class="flex justify-between items-center mb-4">
                <div><h3 class="font-semibold">Per-block Analytics</h3><p class="text-sm text-gray-500 mt-0.5">Research-paper plots — each block scored independently across all 4 tests</p></div>
                <a href="/api/verify/${r.id}/report.pdf" target="_blank" class="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Download Full Report PDF
                </a>
            </div>
            <div class="border-b mb-4 flex gap-1 text-sm">
                ${['moire','correlation','gradient','color'].map((t,i)=>`<button onclick="setAnalyticsTest('${t}')" id="at-${t}" class="atb px-4 py-2 border-b-2 ${i===0?'border-brand-600 text-brand-700 font-medium':'border-transparent text-gray-500 hover:text-gray-700'}">${t.charAt(0).toUpperCase()+t.slice(1)}</button>`).join('')}
                <button onclick="setAnalyticsTest('delta')" id="at-delta" class="atb px-4 py-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700">Δ Delta map</button>
            </div>
            <div id="analytics-pane" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><p class="text-xs text-gray-500 mb-1 text-center">Heatmap (per-block, overlaid)</p><img id="ap-heatmap" src="/api/verify/${r.id}/plot/moire/heatmap.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
                <div><p class="text-xs text-gray-500 mb-1 text-center">Histogram</p><img id="ap-histogram" src="/api/verify/${r.id}/plot/moire/histogram.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
                <div class="md:col-span-2"><p class="text-xs text-gray-500 mb-1 text-center">Per-block scatter</p><img id="ap-scatter" src="/api/verify/${r.id}/plot/moire/scatter.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
            </div>
        </div>
        </div>`;
        window._currentVid = r.id;
...
function setAnalyticsTest(test) {
    const vid = window._currentVid;
    if (!vid) return;
    document.querySelectorAll('.atb').forEach(b => {
        b.className = 'atb px-4 py-2 border-b-2 border-transparent text-gray-500 hover:text-gray-700';
    });
    const active = document.getElementById('at-' + test);
    if (active) active.className = 'atb px-4 py-2 border-b-2 border-brand-600 text-brand-700 font-medium';

    const pane = document.getElementById('analytics-pane');
    if (test === 'delta') {
        pane.innerHTML = `<div class="md:col-span-2"><p class="text-xs text-gray-500 mb-1 text-center">Pixel-level absolute difference: |captured (aligned) − reference|</p><img src="/api/verify/${vid}/plot/moire/delta.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Delta map unavailable (aligned image expired)</div>'"/></div>`;
    } else {
        pane.innerHTML = `
            <div><p class="text-xs text-gray-500 mb-1 text-center">Heatmap (per-block, overlaid)</p><img src="/api/verify/${vid}/plot/${test}/heatmap.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
            <div><p class="text-xs text-gray-500 mb-1 text-center">Histogram</p><img src="/api/verify/${vid}/plot/${test}/histogram.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
            <div class="md:col-span-2"><p class="text-xs text-gray-500 mb-1 text-center">Per-block scatter</p><img src="/api/verify/${vid}/plot/${test}/scatter.png" class="w-full rounded-lg border cursor-pointer" onclick="openImageModal(this.src)" onerror="this.outerHTML='<div class=\\'w-full rounded-lg border bg-gray-100 flex items-center justify-center py-16 text-sm text-gray-400\\'>Plot unavailable</div>'"/></div>
        `;
    }
}

function copyTestResult() {
    const r = window._lastVerifyResult;
    if (!r) { showToast('No result to copy', 'error'); return; }
    const text = `| Verdict | ${r.verdict} |
| Confidence | ${(r.confidence * 100).toFixed(1)}% |
| Moire | ${r.scores.moire.toFixed(3)} |
| Correlation | ${r.scores.correlation.toFixed(3)} |
| Gradient | ${r.scores.gradient.toFixed(3)} |
| Color | ${r.scores.color.toFixed(3)} |
| Markers Found | ${r.markers_found} |
| Alignment | ${r.alignment_method} |
| Notes | ${r.notes || ''} |`;

...
async function pgGen(el) {
    ...
    document.getElementById('gf').addEventListener('submit', async e => {
        ...
        try {
            ...
            document.getElementById('gp').innerHTML = `<div class="flex justify-between items-center mb-4"><h2 class="text-lg font-semibold">Generated Pattern</h2><div class="flex gap-2"><a href="/api/patterns/${r.id}/download" download class="inline-flex items-center gap-1.5 bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Download PNG</a><a href="#/verify" class="inline-flex items-center gap-1.5 bg-white text-brand-700 px-4 py-2 rounded-lg text-sm font-medium border border-brand-200 hover:bg-brand-50 transition">Verify This</a></div></div><div class="flex justify-center"><img src="/api/patterns/${r.id}/preview" class="max-w-md w-full rounded-lg shadow-md cursor-pointer border" onclick="openImageModal(this.src)"/></div><div class="mt-4 grid grid-cols-2 gap-4 text-sm"><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Serial:</span> <span class="font-medium">${r.serial_number}</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Seed:</span> <span class="font-mono font-medium">${r.seed}</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Size:</span> <span class="font-medium">${r.pattern_size}x${r.pattern_size}px</span></div><div class="bg-gray-50 rounded-lg p-3"><span class="text-gray-500">Created:</span> <span class="font-medium">${fmtDate(r.created_at)}</span></div></div>`;
